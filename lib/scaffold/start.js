...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for digicoind is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and digicoincore-node.json. \n\n' +
        ...
        servicesConfig: {
          digicoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/digicoind')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed digicoincore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './digicoincore-node.json');
  ...
  node.on('ready', function() {
    log.info('Digicoincore Node ready');
  });
...
function loadModule(req, service) {
  try {
    // first try in the built-in digicoincore-node services directory
    ...
    if (servicePackage.digicoincoreNode) {
      serviceModule = service.name + '/' + servicePackage.digicoincoreNode;
    }
