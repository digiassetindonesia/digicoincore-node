...
var bitcore = require('digicoincore-lib');
var zmq = require('zmq');
var async = require('async');
var LRU = require('lru-cache');
var DigicoinRPC = require('digicoind-rpc');
...
/**
 * Provides a friendly event driven API to digicoind in Node.js. Manages starting and
 * stopping digicoind as a child process for application support, as well as connecting
 * to multiple digicoind processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Digicoin(options) {
  if (!(this instanceof Digicoin)) {
    return new Digicoin(options);
  }
...
util.inherits(Digicoin, Service);

Digicoin.dependencies = [];

Digicoin.DEFAULT_MAX_TXIDS = 1000;
Digicoin.DEFAULT_MAX_HISTORY = 50;
Digicoin.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Digicoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Digicoin.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Digicoin.DEFAULT_SPAWN_RESTART_TIME = 5000;
Digicoin.DEFAULT_SPAWN_STOP_TIME = 10000;
Digicoin.DEFAULT_TRY_ALL_INTERVAL = 1000;
Digicoin.DEFAULT_REINDEX_INTERVAL = 10000;
Digicoin.DEFAULT_START_RETRY_INTERVAL = 5000;
Digicoin.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Digicoin.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Digicoin.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Digicoin.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Digicoin.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Digicoin.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Digicoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Digicoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Digicoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Digicoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Digicoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Digicoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Digicoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Digicoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Digicoin.prototype._initCaches = function() {
...
Digicoin.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Digicoin.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Digicoin.prototype.getPublishEvents = function() {
  return [
    {
...
Digicoin.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'digicoind/' + name, 'total:', this.subscriptions[name].length);
};

Digicoin.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'digicoind/' + name, 'total:', this.subscriptions[name].length);
};


Digicoin.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
Digicoin.prototype.unsubscribeBalance = function(emitter, addresses) {
...
Digicoin.prototype.unsubscribeBalanceAll = function(emitter) {
...
Digicoin.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'digicoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Digicoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'digicoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Digicoin.prototype.unsubscribeAddressAll = function(emitter) {
...
Digicoin.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Digicoin.DEFAULT_CONFIG_SETTINGS;
...
Digicoin.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
Digicoin.prototype._expandRelativeDatadir = function() {
...
Digicoin.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in digicoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in digicoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in digicoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './digicoin.conf');

  log.info('Using digicoin config file:', configPath);
...
Digicoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of digicoincore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of digicoincore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of digicoincore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to digicoind from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from digicoind. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from digicoind. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that digicoind is undergoing a reindex. ' +
             ...
             'of digicoincore-node services will start.');
    node._reindex = true;
  }
};

Digicoin.prototype._resetCaches = function() {
...
Digicoin.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Digicoin.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Digicoin.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Digicoin Daemon Ready');
          callback();
        });
      });
...
Digicoin.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8332
  };
...
Digicoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/digicoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/digicoin.conf';
    }
  }
  return networkPath;
};

Digicoin.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Digicoin.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('digicoind/hashblock', message.toString('hex'));
    }
  }

};

Digicoin.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Digicoin.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Digicoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Digicoin.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Digicoin.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Digicoin.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
Digicoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('digicoind/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Digicoin.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('digicoind/rawtransaction', message.toString('hex'));
    }
...
Digicoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Digicoin.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Digicoin.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Digicoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Digicoin.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Digicoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Digicoin.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Digicoin.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Digicoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/digicoind.pid';
...
Digicoin.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting digicoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Digicoin process unexpectedly exited with code:', code);
        log.warn('Restarting digicoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Digicoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn digicoind.'));
      }
...
Digicoin.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to digicoind.'));
    }
 ...
 */
Digicoin.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Digicoin configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Digicoin.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Digicoin.prototype.syncPercentage = function(callback) {
  var self = this;
...
Digicoin.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, digicoincore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Digicoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Digicoin.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Digicoin.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Digicoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in digicoincore is most recent to less recent
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Digicoin.prototype._getConfirmationsDetail = function(transaction) {
...
Digicoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Digicoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Digicoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A digicoincoin transaction id
 * @param {Function} callback
 */
Digicoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Digicoin.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Digicoin.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Digicoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
Digicoin.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Digicoin.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
Digicoin.prototype.estimateSmartFee = function (blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Digicoin.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Digicoincore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Digicoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getInfo = function(callback) {
...
Digicoin.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Digicoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('digicoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('digicoind process did not exit'));
      }
 ...
 * @param {Function} callback
 */
Digicoin.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Digicoin.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
Digicoin.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
Digicoin.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
Digicoin.prototype.getStakingInfo = function(callback) {
    var self = this,
