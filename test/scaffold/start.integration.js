...
var DigicoinService = require('../../lib/services/digicoind');
...
describe('#start', function() {
  ...
  describe('will dynamically create a node from a configuration', function() {
    ...
    it('require each digicoincore-node service with default config', function(done) {
      var node;
      var TestNode = function(options) {
        options.services[0].should.deep.equal({
          name: 'digicoind',
          module: DigicoinService,
          config: {
            spawn: {
              datadir: './data'
            }
    ...
    it('require each digicoincore-node service with explicit config', function(done) {
      var node;
      var TestNode = function(options) {
        options.services[0].should.deep.equal({
          name: 'digicoind',
