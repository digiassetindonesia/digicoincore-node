...
var Transaction = digicoincore.Transaction;
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/digicoin.conf')));
var DigicoinService = proxyquire('../../lib/services/digicoind', {
	fs: {
		readFileSync: readFileSync
	}
});
var defaultDigicoinConf = fs.readFileSync(path.resolve(__dirname, '../data/default.digicoin.conf'), 'utf8');

describe('Digicoin Service', function() {
	...
	var baseConfig = {
		node: {
			network: digicoincore.Networks.testnet
		},
		spawn: {
			datadir: 'testdir',
			exec: 'testpath'
	...
	describe('@constructor', function() {
		it('will create an instance', function() {
			var digicoind = new DigicoinService(baseConfig);
			should.exist(digicoind);
		});
		it('will create an instance without `new`', function() {
			var digicoind = DigicoinService(baseConfig);
			should.exist(digicoind);
		});
		it('will init caches', function() {
			var digicoind = new DigicoinService(baseConfig);

			should.exist(digicoind.utxosCache);
			should.exist(digicoind.txidsCache);
			should.exist(digicoind.balanceCache);
			should.exist(digicoind.summaryCache);
			should.exist(digicoind.transactionDetailedCache);
			should.exist(digicoind.accountInfo);

			should.exist(digicoind.transactionCache);
			should.exist(digicoind.rawTransactionCache);
			should.exist(digicoind.rawJsonTransactionCache);
			should.exist(digicoind.blockCache);
			should.exist(digicoind.blockJsonCache);
			should.exist(digicoind.blockSubsidyCache);
			should.exist(digicoind.rawBlockCache);
			should.exist(digicoind.blockHeaderCache);
			should.exist(digicoind.zmqKnownTransactions);
			should.exist(digicoind.zmqKnownBlocks);
			should.exist(digicoind.lastTip);
			should.exist(digicoind.lastTipTimeout);
			should.equal(digicoind.dgpInfoCache, null, 'should be null');
			should.equal(digicoind.miningInfoCache, null, 'should be null');
			should.equal(digicoind.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(digicoind.maxTxids, 1000);
			should.equal(digicoind.maxTransactionHistory, 50);
			should.equal(digicoind.maxAddressesQuery, 10000);
			should.equal(digicoind.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(digicoind.spawnRestartTime, 5000);
			should.equal(digicoind.spawnStopTime, 10000);

			// try all interval
			should.equal(digicoind.tryAllInterval, 1000);
			should.equal(digicoind.startRetryInterval, 5000);

			// rpc limits
			should.equal(digicoind.transactionConcurrency, 5);

			// sync progress level when zmq subscribes to events
			should.equal(digicoind.zmqSubscribeProgress, 0.9999);
		});
		it('will init clients', function() {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.nodes.should.deep.equal([]);
			digicoind.nodesIndex.should.equal(0);
			digicoind.nodes.push({ client: sinon.stub() });
			should.exist(digicoind.client);
		});
		it('will set subscriptions', function() {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.subscriptions.should.deep.equal({
				address: {},
				rawtransaction: [],
				hashblock: [],
				balance: {},
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			var digicoind = new DigicoinService(baseConfig);
			digicoind._initDefaults({ transactionConcurrency: 10 });
			digicoind.transactionConcurrency.should.equal(10);
			digicoind._initDefaults({});
			digicoind.transactionConcurrency.should.equal(5);
		});
	});

	describe('@dependencies', function() {
		it('will have no dependencies', function() {
			DigicoinService.dependencies.should.deep.equal([]);
		});
	});
	...
	describe('#getAPIMethods', function() {
		it('will return spec', function() {
			var digicoind = new DigicoinService(baseConfig);
			var methods = digicoind.getAPIMethods();
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			var digicoind = new DigicoinService(baseConfig);
			var events = digicoind.getPublishEvents();
			should.exist(events);
			events.length.should.equal(4);

			events[0].name.should.equal('digicoind/rawtransaction');
			events[0].scope.should.equal(digicoind);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('digicoind/hashblock');
			events[1].scope.should.equal(digicoind);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('digicoind/addresstxid');
			events[2].scope.should.equal(digicoind);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('digicoind/addressbalance');
			events[3].scope.should.equal(digicoind);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.subscribe = sinon.stub();
			digicoind.unsubscribe = sinon.stub();
			var events = digicoind.getPublishEvents();

			events[0].subscribe('test');
			digicoind.subscribe.args[0][0].should.equal('rawtransaction');
			digicoind.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			digicoind.unsubscribe.args[0][0].should.equal('rawtransaction');
			digicoind.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			digicoind.subscribe.args[1][0].should.equal('hashblock');
			digicoind.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			digicoind.unsubscribe.args[1][0].should.equal('hashblock');
			digicoind.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			var digicoind = new DigicoinService(baseConfig);
			var emitter = {};
			digicoind.subscribe('hashblock', emitter);
			digicoind.subscriptions.hashblock[0].should.equal(emitter);

			var emitter2 = {};
			digicoind.subscribe('rawtransaction', emitter2);
			digicoind.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			digicoind.subscribe('hashblock', emitter1);
			digicoind.subscribe('hashblock', emitter2);
			digicoind.subscribe('hashblock', emitter3);
			digicoind.subscribe('hashblock', emitter4);
			digicoind.subscribe('hashblock', emitter5);
			digicoind.subscriptions.hashblock.length.should.equal(5);

			digicoind.unsubscribe('hashblock', emitter3);
			digicoind.subscriptions.hashblock.length.should.equal(4);
			digicoind.subscriptions.hashblock[0].should.equal(emitter1);
			digicoind.subscriptions.hashblock[1].should.equal(emitter2);
			digicoind.subscriptions.hashblock[2].should.equal(emitter4);
			digicoind.subscriptions.hashblock[3].should.equal(emitter5);
		});
		it('will not remove item an already unsubscribed item', function() {
			var digicoind = new DigicoinService(baseConfig);
			var emitter1 = {};
			var emitter3 = {};
			digicoind.subscriptions.hashblock = [emitter1];
			digicoind.unsubscribe('hashblock', emitter3);
			digicoind.subscriptions.hashblock.length.should.equal(1);
			digicoind.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will not an invalid address', function() {
			...
			digicoind.subscribeAddress(emitter, ['invalidaddress']);
			should.not.exist(digicoind.subscriptions.address['invalidaddress']);
		});
		it('will add a valid address', function() {
			var digicoind = new DigicoinService(baseConfig);
			var emitter = new EventEmitter();
			digicoind.subscribeAddress(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will handle multiple address subscribers', function() {
			var digicoind = new DigicoinService(baseConfig);
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			digicoind.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			digicoind.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will not add the same emitter twice', function() {
			...
			digicoind.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			digicoind.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			digicoind.unsubscribeAddress(emitter1);
			digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			digicoind.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			digicoind.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
		});
		it('will remove empty addresses', function() {
			...
			digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			digicoind.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			digicoind.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			digicoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(digicoind, 'unsubscribeAddressAll');
			digicoind.unsubscribeAddress(emitter1);
			digicoind.unsubscribeAddressAll.callCount.should.equal(1);
			digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			digicoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			digicoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			digicoind.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			digicoind.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			digicoind.unsubscribeAddressAll(emitter1);
			digicoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			digicoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			digicoind.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(digicoind.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_getDefaultConfig', function() {
		it('will generate config file from defaults', function() {
			var digicoind = new DigicoinService(baseConfig);
			var config = digicoind._getDefaultConfig();
			config.should.equal(defaultDigicoinConf);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a digicoin.conf file', function() {
			var TestDigicoin = proxyquire('../../lib/services/digicoind', {
				fs: {
					readFileSync: readFileSync,
			...
			var digicoind = new TestDigicoin(baseConfig);
			digicoind.options.spawn.datadir = '/tmp/.digicoin';

			sinon.spy(digicoind, '_expandRelativeDatadir');
			sinon.spy(digicoind, '_checkConfigIndexes');
			sinon.spy(digicoind, '_getDefaultConf')
			sinon.spy(digicoind, '_getNetworkConfigPath');
			sinon.spy(digicoind, '_parseBitcoinConf');

			var node = {};
			digicoind._loadSpawnConfiguration(node);

			digicoind._expandRelativeDatadir.callCount.should.equal(1);
			digicoind._checkConfigIndexes.callCount.should.equal(1);
			digicoind._getDefaultConf.callCount.should.equal(1);
			digicoind._getNetworkConfigPath.callCount.should.equal(1);
			digicoind._parseBitcoinConf.callCount.should.equal(2);

			should.exist(digicoind.spawn.config);
			digicoind.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: digicoincore.Networks.testnet,
					configPath: '/tmp/.digicoincore/digicoincore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			var digicoind = new TestDigicoin(config);
			...
			digicoind.options.spawn.datadir = './data';
			var node = {};
			digicoind._loadSpawnConfiguration(node);
			...
			digicoind.options.spawn.datadir.should.equal('/tmp/.digicoincore/data');
		});
		it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
			var TestDigicoin = proxyquire('../../lib/services/digicoind', {
				fs: {
					readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/baddigicoin.conf')),
					existsSync: sinon.stub().returns(true),
				},
				mkdirp: {
					sync: sinon.stub()
			...
			var digicoind = new TestDigicoin(baseConfig);
			(function() {
				digicoind._loadSpawnConfiguration({ datadir: './test' });
			}).should.throw(digicoincore.errors.InvalidState);
		});
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the digicoin.conf file', function() {
			var digicoind = new DigicoinService(baseConfig);
			var config = {
				txindex: 0,
				addressindex: 1,
			...
			var node = {};
			(function() {
				digicoind._checkConfigIndexes(config, node);
			}).should.throw('"txindex" option');
		});
		it('should warn the user if addressindex isn\'t set to 1 in the digicoin.conf file', function() {
			var digicoind = new DigicoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 0,
		...
		it('should warn the user if spentindex isn\'t set to 1 in the digicoin.conf file', function() {
			var digicoind = new DigicoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if server isn\'t set to 1 in the digicoin.conf file', function() {
			var digicoind = new DigicoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if reindex is set to 1 in the digicoin.conf file', function() {
			var digicoind = new DigicoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				digicoind.transactionDetailedCache.set(keys[i], {});
				digicoind.utxosCache.set(keys[i], {});
				digicoind.accountInfo.set(keys[i], {});
				digicoind.txidsCache.set(keys[i], {});
				digicoind.balanceCache.set(keys[i], {});
				digicoind.summaryCache.set(keys[i], {});
				digicoind.blockOverviewCache.set(keys[i], {});
				digicoind.blockJsonCache.set(keys[i], {});
			}
			digicoind._resetCaches();
			should.equal(digicoind.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(digicoind.utxosCache.get(keys[0]), undefined);
			should.equal(digicoind.accountInfo.get(keys[0]), undefined);
			should.equal(digicoind.txidsCache.get(keys[0]), undefined);
			should.equal(digicoind.balanceCache.get(keys[0]), undefined);
			should.equal(digicoind.summaryCache.get(keys[0]), undefined);
			should.equal(digicoind.blockOverviewCache.get(keys[0]), undefined);
			should.equal(digicoind.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.tryAllInterval = 1;
			digicoind.nodes.push({
				client: {
			...
			digicoind._tryAllClients(function(client, next) {
				client.getInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				digicoind.nodes[0].client.getInfo.callCount.should.equal(1);
				digicoind.nodes[1].client.getInfo.callCount.should.equal(1);
				digicoind.nodes[2].client.getInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			digicoind._tryAllClients(function(client, next) {
				...
				digicoind.nodes[1].client.getInfo.callCount.should.equal(1);
				digicoind.nodes[2].client.getInfo.callCount.should.equal(1);
				digicoind.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_wrapRPCError', function() {
		it('will convert digicoind-rpc error object into JavaScript error', function() {
			var digicoind = new DigicoinService(baseConfig);
			var error = digicoind._wrapRPCError({ message: 'Test error', code: -1 });
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			digicoind.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
			digicoind.nodes.push({
				client: {
			...
			digicoind._initChain(function() {
				log.info.callCount.should.equal(1);
				digicoind.getRawBlock.callCount.should.equal(1);
				digicoind.getRawBlock.args[0][0].should.equal('genesishash');
				digicoind.height.should.equal(5000);
				digicoind.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		it('it will handle error from getBestBlockHash', function(done) {
			...
			digicoind._initChain(function(err) {
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			digicoind.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			digicoind._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_getDefaultConf', function() {
		afterEach(function() {
			digicoincore.Networks.disableRegtest();
			baseConfig.node.network = digicoincore.Networks.testnet;
		});
		it('will get default rpc port for livenet', function() {
			var config = {
				node: {
					network: digicoincore.Networks.livenet
				},
				spawn: {
					datadir: 'testdir',
					exec: 'testpath'
			...
			var digicoind = new DigicoinService(config);
			digicoind._getDefaultConf().rpcport.should.equal(8332);
		});
		it('will get default rpc port for testnet', function() {
			...
			var digicoind = new DigicoinService(config);
			digicoind._getDefaultConf().rpcport.should.equal(18332);
		});
	...
	describe('#_getNetworkConfigPath', function() {
		...
		it('will get default config path for livenet', function() {
			...
			var digicoind = new DigicoinService(config);
			should.equal(digicoind._getNetworkConfigPath(), undefined);
		});
		it('will get default rpc port for testnet', function() {
			var config = {
				node: {
			...
			var digicoind = new DigicoinService(config);
			digicoind._getNetworkConfigPath().should.equal('testnet3/digicoin.conf');
		});
		it('will get default rpc port for regtest', function() {
			digicoincore.Networks.enableRegtest();
			var config = {
			...
			var digicoind = new DigicoinService(config);
			digicoind._getNetworkConfigPath().should.equal('regtest/digicoin.conf');
		});
	});
	...
	describe('#_getNetworkOption', function() {
		...
		it('return --testnet for testnet', function() {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.node.network = digicoincore.Networks.testnet;
			digicoind._getNetworkOption().should.equal('--testnet');
		});
		it('return --regtest for testnet', function() {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.node.network = digicoincore.Networks.testnet;
			digicoincore.Networks.enableRegtest();
			digicoind._getNetworkOption().should.equal('--regtest');
		});
		it('return undefined for livenet', function() {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.node.network = digicoincore.Networks.livenet;
			digicoincore.Networks.enableRegtest();
			should.equal(digicoind._getNetworkOption(), undefined);
		});
	});
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			digicoind._rapidProtectedUpdateTip = sinon.stub();
			digicoind.on('block', function(block) {
				block.should.equal(message);
				done();
			});
			digicoind._zmqBlockHandler(node, message);
			digicoind._rapidProtectedUpdateTip.callCount.should.equal(1);
		});
		it('will not emit same block twice', function(done) {
			...
			var message = new Buffer('76843f1dfe455347d897e757ff5ff51a16a0f62b5d02a659c5680392de3a2b89', 'hex');
			digicoind._rapidProtectedUpdateTip = sinon.stub();
			sinon.spy(digicoind, 'emit');
			digicoind.on('block', function(block) {
				block.should.equal(message);
				digicoind.emit.callCount.should.equal(1);
				digicoind._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			digicoind._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			digicoind._zmqBlockHandler(node, message);
			digicoind._rapidProtectedUpdateTip.callCount.should.equal(1);
			digicoind._rapidProtectedUpdateTip.args[0][0].should.equal(node);
			digicoind._rapidProtectedUpdateTip.args[0][1].should.equal(message);
		});
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(digicoind, 'emit');
			var emitter = new EventEmitter();
			digicoind.subscriptions.hashblock.push(emitter);
			emitter.on('digicoind/hashblock', function(blockHash) {
				digicoind._rapidProtectedUpdateTip.callCount.should.equal(1);
				digicoind.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			...
			var count = 0;
			function repeat() {
				digicoind._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.syncPercentage = sinon.stub();
			sinon.spy(digicoind, '_resetCaches');
			digicoind.on('error', function(err) {
				err.code.should.equal(-1);
				err.message.should.equal('Test error');
				node.client.getBlock.callCount.should.equal(1);
				digicoind._resetCaches.callCount.should.equal(1);
			...
			digicoind._updateTip(node, message);
		});
		it('emit synced if percentage is 100', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
			digicoind.on('synced', function() {
				digicoind.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
				done();
			});
			var node = {
		...
		it('NOT emit synced if percentage is less than 100', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
			digicoind.on('synced', function() {
				digicoind.syncPercentage.callCount.should.equal(0);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('log and emit error from syncPercentage', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			digicoind.on('error', function(err) {
				log.error.callCount.should.equal(1);
				digicoind.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('reset caches and set height', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.syncPercentage = sinon.stub();
			digicoind._resetCaches = sinon.stub();
			digicoind.on('tip', function(height) {
				digicoind._resetCaches.callCount.should.equal(1);
				height.should.equal(10);
				digicoind.height.should.equal(10);
				done();
			});
			var node = {
				client: {
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using digicoincore.Transaction', function() {
			var digicoind = new DigicoinService(baseConfig);
			var wif = 'cVTiUNavxbCzuoaQboDcgEhKYHhgXRaeQ8ZRoYkVXE9mxT1TcSrc';
			var privkey = digicoincore.PrivateKey.fromWIF(wif);
			var inputAddress = privkey.toAddress(digicoincore.Networks.testnet);
			var outputAddress = digicoincore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = digicoincore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: digicoincore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = digicoind._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			var digicoind = new DigicoinService(baseConfig);
			var tx = digicoincore.Transaction();
			tx.addInput(digicoincore.Transaction.Input({
				prevTxId: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				script: digicoincore.Script('OP_TRUE'),
				outputIndex: 1,
				output: {
					script: digicoincore.Script('OP_TRUE'),
					satoshis: 5000000000
			...
			tx.addOutput(digicoincore.Transaction.Output({
				script: digicoincore.Script('OP_TRUE'),
				satoshis: 5000000000
			}));
			var addresses = digicoind._getAddressesFromTransaction(tx);
		...
		it('will return unique values', function() {
			var digicoind = new DigicoinService(baseConfig);
			var tx = digicoincore.Transaction();
			var address = digicoincore.Address('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX');
			tx.addOutput(digicoincore.Transaction.Output({
				script: digicoincore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(digicoincore.Transaction.Output({
				script: digicoincore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			digicoind._getAddressesFromTransaction = sinon.stub().returns([address]);
			var emitter = new EventEmitter();
			digicoind.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('digicoind/addresstxid', function(data) {
			...
			digicoind._notifyAddressTxidSubscribers(txid, transaction);
		});
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			var emitter = new EventEmitter();
			digicoind.subscriptions.rawtransaction.push(emitter);
			emitter.on('digicoind/rawtransaction', function(hex) {
				hex.should.be.a('string');
				hex.should.equal(expectedBuffer.toString('hex'));
				done();
			});
			var node = {};
			digicoind._zmqTransactionHandler(node, expectedBuffer);
		});
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(digicoind, 'emit');
			digicoind.subscriptions.rawtransaction.push(emitter);
			emitter.on('digicoind/rawtransaction', function() {
				done();
			});
			var node = {};
			digicoind._zmqTransactionHandler(node, expectedBuffer);
		...
		it('will emit "tx" event', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			var expectedBuffer = new Buffer(txhex, 'hex');
			digicoind.on('tx', function(buffer) {
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind._updateTip = sinon.stub();
			digicoind._subscribeZmqEvents = sinon.stub();
			var blockEvents = 0;
			digicoind.on('block', function() {
				blockEvents++;
			});
			var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
			...
			digicoind._checkSyncedAndSubscribeZmqEvents(node);
			setTimeout(function() {
				...
				digicoind._updateTip.callCount.should.equal(11);
				digicoind._subscribeZmqEvents.callCount.should.equal(1);
				done();
			}, 200);
		});
	...
	describe('#_subscribeZmqEvents', function() {
		it('will call subscribe on zmq socket', function() {
			...
			digicoind._subscribeZmqEvents(node);
		...
		it('will call relevant handler for rawtx topics', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind._zmqTransactionHandler = sinon.stub();
			var node = {
			...
			node.zmqSubSocket.on('message', function() {
				digicoind._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind._zmqBlockHandler = sinon.stub();
			var node = {
				zmqSubSocket: new EventEmitter()
			};
			node.zmqSubSocket.subscribe = sinon.stub();
			digicoind._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				digicoind._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			digicoind._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				digicoind._zmqBlockHandler.callCount.should.equal(0);
				digicoind._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_initZmqSubSocket', function() {
		it('will setup zmq socket', function() {
			...
			var digicoind = new DigicoinService(baseConfig);
			var node = {};
			digicoind._initZmqSubSocket(node, 'url');
	...
	describe('#_checkReindex', function() {
		...
		it('give error from client getblockchaininfo', function(done) {
			...
			digicoind._checkReindex(node, function(err) {
				should.exist(err);
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			digicoind._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			digicoind._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
		...
		it('will set height and emit tip', function(done) {
			...
			digicoind.on('tip', function(height) {
				...
				digicoind.height.should.equal(100);
				done();
			});
			digicoind._loadTipFromNode(node, function(err) {
				if (err) {
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			var TestDigicoinService = proxyquire('../../lib/services/digicoind', {
				fs: {
					readFile: readFile
				}
			});
			var digicoind = new TestDigicoinService(baseConfig);
			digicoind.spawnStopTime = 1;
			digicoind._process = {};
			digicoind._process.kill = sinon.stub();
			digicoind._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				digicoind._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			digicoind._process.kill = sinon.stub().throws(error2);
			digicoind._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind._loadSpawnConfiguration = sinon.stub();
			digicoind._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
			digicoind._spawnChildProcess(function(err) {
				digicoind._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			var digicoind = new DigicoinService(baseConfig);
			digicoind._loadSpawnConfiguration = sinon.stub();
			digicoind._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
			digicoind._spawnChildProcess(function(err) {
				digicoind._loadSpawnConfiguration.callCount.should.equal(1);
				digicoind._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		it('will exit spawn if shutdown', function() {
			...
			var digicoind = new TestDigicoinService(config);
			digicoind.spawn = {};
			digicoind._loadSpawnConfiguration = sinon.stub();
			digicoind._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
			digicoind.node.stopping = true;
			digicoind._spawnChildProcess(function(err) {
				err.should.be.instanceOf(Error);
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			digicoind._loadSpawnConfiguration = sinon.stub();
			digicoind.spawn = {};
			digicoind.spawn.exec = 'testexec';
			digicoind.spawn.configPath = 'testdir/digicoin.conf';
			digicoind.spawn.datadir = 'testdir';
			digicoind.spawn.config = {};
			digicoind.spawn.config.rpcport = 20001;
			digicoind.spawn.config.rpcuser = 'digicoin';
			digicoind.spawn.config.rpcpassword = 'password';
			digicoind.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			digicoind._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			digicoind._initZmqSubSocket = sinon.stub();
			digicoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			digicoind._checkReindex = sinon.stub().callsArgWith(1, null);
			digicoind._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/digicoin.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				digicoind._loadTipFromNode.callCount.should.equal(1);
				digicoind._initZmqSubSocket.callCount.should.equal(1);
				should.exist(digicoind._initZmqSubSocket.args[0][0].client);
				digicoind._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				digicoind._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(digicoind._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
				should.exist(node.client);
				done();
			});
		});
		it('will respawn digicoind spawned process', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestDigicoinService = proxyquire('../../lib/services/digicoind', {
				fs: {
			...
			digicoind._loadSpawnConfiguration = sinon.stub();
			digicoind.spawn = {};
			digicoind.spawn.exec = 'digicoind';
			digicoind.spawn.datadir = '/tmp/digicoin';
			digicoind.spawn.configPath = '/tmp/digicoin/digicoin.conf';
			digicoind.spawn.config = {};
			digicoind.spawnRestartTime = 1;
			digicoind._loadTipFromNode = sinon.stub().callsArg(1);
			digicoind._initZmqSubSocket = sinon.stub();
			digicoind._checkReindex = sinon.stub().callsArg(1);
			digicoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			digicoind._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			sinon.spy(digicoind, '_spawnChildProcess');
			digicoind._spawnChildProcess(function(err) {
				if (err) {
					return done(err);
				}
				process.once('exit', function() {
					setTimeout(function() {
						digicoind._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will emit error during respawn', function(done) {
			...
			digicoind._spawnChildProcess(function(err) {
				...
				digicoind._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
				digicoind.on('error', function(err) {
					err.should.be.instanceOf(Error);
					err.message.should.equal('test');
					done();
		...
		it('will NOT respawn digicoind spawned process if shutting down', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestDigicoinService = proxyquire('../../lib/services/digicoind', {
				fs: {
			...
			digicoind._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						digicoind._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			var digicoind = new TestDigicoinService(baseConfig);
			digicoind.startRetryInterval = 1;
			digicoind._loadSpawnConfiguration = sinon.stub();
			digicoind.spawn = {};
			digicoind.spawn.exec = 'testexec';
			digicoind.spawn.configPath = 'testdir/digicoin.conf';
			...
			digicoind.spawn.config.rpcpassword = 'password';
			digicoind.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
			digicoind._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
			digicoind._spawnChildProcess(function(err) {
				digicoind._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			digicoind._initZmqSubSocket = sinon.stub();
			digicoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			digicoind._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			var digicoind = new DigicoinService(config);
			digicoind.node.stopping = true;
			digicoind.startRetryInterval = 100;
			digicoind._loadTipFromNode = sinon.stub();
			digicoind._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to digicoind/);
				digicoind._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			digicoind.startRetryInterval = 1;
			var config = {};
			digicoind._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				digicoind._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			digicoind._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			digicoind._connectProcess(config, function(err, node) {
				should.not.exist(err);
				digicoind._loadTipFromNode.callCount.should.equal(1);
				digicoind._initZmqSubSocket.callCount.should.equal(1);
				digicoind._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error if "spawn" and "connect" are both not configured', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.options = {};
			digicoind.start(function(err) {
				err.should.be.instanceof(Error);
				err.message.should.be.a('string').and.equal('Digicoin configuration options "spawn" or "connect" are expected');
				done();
			});
		});
		...
		it('will give error from connectProcess', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
			digicoind.options = {
				connect: [
					{}
				]
			...
			digicoind.start(function(err) {
				digicoind._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			var node = {};
			digicoind._initChain = sinon.stub().callsArg(0);
			digicoind._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
			digicoind.options = {
				spawn: {}
			};
			digicoind.start(function(err) {
				should.not.exist(err);
				digicoind.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(digicoind, '_spawnChildProcess');
			var nodes = [{}];
			digicoind._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			digicoind.options = {
				connect: [
					{}
				]
			...
			digicoind.start(function(err) {
				should.not.exist(err);
				digicoind._initChain.callCount.should.equal(1);
				digicoind._connectProcess.callCount.should.equal(1);
				digicoind.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#isSynced', function() {
		it('will give error from syncPercentage', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			digicoind.isSynced(function(err) {
				should.exist(err);
				digicoind.syncPercentage.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
		...
		it('will give "true" if percentage is 100.00', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
			digicoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
				digicoind.syncPercentage.callCount.should.equal(1);
		...
		it('will give "true" if percentage is 99.98', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
			digicoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 99.49', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
			digicoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 1', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
			digicoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
	...
	describe('#syncPercentage', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.syncPercentage(function(err) {
				should.exist(err);
				digicoind.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
		...
		it('will call client getInfo and give result', function(done) {
			...
			digicoind.syncPercentage(function(err, percentage) {
				if (err) {
					return done(err);
				}
				digicoind.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
	...
	describe('#_normalizeAddressArg', function() {
		it('will turn single address into array', function() {
			var digicoind = new DigicoinService(baseConfig);
			var args = digicoind._normalizeAddressArg('address');
			args.should.deep.equal(['address']);
		});
		it('will keep an array as an array', function() {
			var digicoind = new DigicoinService(baseConfig);
			var args = digicoind._normalizeAddressArg(['address one', 'address two']);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			sinon.spy(digicoind, '_normalizeAddressArg');
			digicoind.nodes.push({
				client: {
			...
			var options = {};
			digicoind.getAddressBalance(address, options, function(err) {
				digicoind._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			digicoind.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				digicoind.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					digicoind._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			digicoind.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = digicoind._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = digicoind._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = digicoind._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = digicoind._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				digicoind._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		it('will give error from _getHeightRangeQuery', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
			digicoind.getAddressTxids('address', {}, function(err) {
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give rpc error from mempool query', function() {
			...
			sinon.spy(digicoind, '_getHeightRangeQuery');
			sinon.spy(digicoind, '_normalizeAddressArg');
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			digicoind.getAddressTxids(address, options, function(err) {
				should.exist(err);
				digicoind._getHeightRangeQuery.callCount.should.equal(1);
				digicoind._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			digicoind.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				digicoind._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			digicoind.getAddressTxids(address, options, function(err, txids) {
				...
				digicoind.getAddressTxids(address, options, function(err, txids) {
					...
					digicoind._getHeightRangeQuery.callCount.should.equal(2);
					digicoind._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			digicoind.getAddressTxids(address, options, function(err, txids) {
				...
				digicoind.getAddressTxids(address, options, function(err, txids) {
					...
					digicoind._getHeightRangeQuery.callCount.should.equal(4);
					digicoind._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			digicoind.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				digicoind.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					digicoind.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						digicoind._getHeightRangeQuery.callCount.should.equal(3);
						digicoind._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getConfirmationDetail', function() {
		...
		it('should get 0 confirmation', function() {
			...
			var digicoind = new DigicoinService(baseConfig);
			digicoind.height = 10;
			var confirmations = digicoind._getConfirmationsDetail(tx);
	...
	describe('#_getAddressDetailsForInput', function() {
		it('will return if missing an address', function() {
			var digicoind = new DigicoinService(baseConfig);
			var result = {};
			digicoind._getAddressDetailsForInput({}, 0, result, []);
		...
		it('will only add address if it matches', function() {
			var digicoind = new DigicoinService(baseConfig);
			var result = {};
			digicoind._getAddressDetailsForInput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForOutput', function() {
		it('will return if missing an address', function() {
			var digicoind = new DigicoinService(baseConfig);
			var result = {};
			digicoind._getAddressDetailsForOutput({}, 0, result, []);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will only add address if it matches', function() {
			var digicoind = new DigicoinService(baseConfig);
			var result = {};
			digicoind._getAddressDetailsForOutput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			sinon.spy(digicoind, '_getAddressDetailsForInput');
			sinon.spy(digicoind, '_getAddressDetailsForOutput');

			var addresses = ['qey8KJ8xkyLUUqFZZwqngXX9U2q9ef6Tw4'];
			var details = digicoind._getAddressDetailsForTransaction(tx, addresses);
			...
			digicoind._getAddressDetailsForInput.callCount.should.equal(1);
			digicoind._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			var digicoind = new DigicoinService(baseConfig);
			sinon.spy(digicoind, '_getConfirmationsDetail');
			digicoind.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
			digicoind.height = 300;
			var addresses = {};
			digicoind._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			digicoind._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				digicoind.getDetailedTransaction.callCount.should.equal(1);
				digicoind._getAddressDetailsForTransaction.callCount.should.equal(1);
				digicoind._getConfirmationsDetail.callCount.should.equal(1);
		...
		it('give error from _getDetailedTransaction', function(done) {
			...
			var digicoind = new DigicoinService(baseConfig);
			digicoind.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
			digicoind._getAddressDetailedTransaction(txid, {}, function(err) {
				err.should.be.instanceof(Error);
				digicoind.getDetailedTransaction.callCount.should.equal(1);
				done();
			});
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from digicoincore addresses', function() {
			var addresses = [
				digicoincore.Address('qU12Fa5RHM535kSDvywxPjCmbL7gwkQJZ6'),
				digicoincore.Address('qcSLSxN1sngCWSrKFZ6UC7ri4hhVSdq9SU'),
			];
			var digicoind = new DigicoinService(baseConfig);
			var strings = digicoind._getAddressStrings(addresses);
		...
		it('will give error with unknown', function() {
			...
			var digicoind = new DigicoinService(baseConfig);
			(function() {
				digicoind._getAddressStrings(addresses);
	...
	describe('#getAddressHistory', function() {
		...
		it('will give an error if length of addresses is too long', function(done) {
			...
			var digicoind = new DigicoinService(baseConfig);
			sinon.spy(digicoind, '_normalizeAddressArg');
			digicoind.maxAddressesQuery = 100;
			digicoind.getAddressHistory(addresses, {}, function(err) {
				should.exist(err);
				digicoind._normalizeAddressArg.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			sinon.spy(digicoind, '_normalizeAddressArg');
			sinon.spy(digicoind, '_getAddressStrings');
			digicoind.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				digicoind._normalizeAddressArg.callCount.should.equal(1);
				digicoind._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			sinon.spy(digicoind, '_normalizeAddressArg');
			sinon.spy(digicoind, '_getAddressStrings');
			digicoind.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
			digicoind.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				digicoind._normalizeAddressArg.callCount.should.equal(1);
				digicoind._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			sinon.spy(digicoind, '_normalizeAddressArg');
			sinon.spy(digicoind, '_getAddressStrings');
			digicoind.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
			digicoind._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			digicoind.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				digicoind._normalizeAddressArg.callCount.should.equal(1);
				digicoind._getAddressStrings.callCount.should.equal(1);
				digicoind.getAddressTxids.callCount.should.equal(1);
				digicoind._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			sinon.spy(digicoind, '_normalizeAddressArg');
			sinon.spy(digicoind, '_getAddressStrings');
			sinon.spy(digicoind, 'getAddressTxids');
			digicoind.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			digicoind.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				digicoind._normalizeAddressArg.callCount.should.equal(1);
				digicoind._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			sinon.spy(digicoind, '_normalizeAddressArg');
			sinon.spy(digicoind, '_getAddressStrings');
			sinon.spy(digicoind, '_paginateTxids');
			digicoind._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(digicoind, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			digicoind.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			digicoind.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				digicoind._normalizeAddressArg.callCount.should.equal(1);
				digicoind._getAddressStrings.callCount.should.equal(1);
				digicoind.getAddressTxids.callCount.should.equal(1);
				digicoind._getAddressDetailedTransaction.callCount.should.equal(2);
				digicoind._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			digicoind.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			digicoind.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			digicoind.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			digicoind.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			digicoind.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			digicoind.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			digicoind.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			digicoind.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			digicoind.getAddressSummary(address, options, function(err, summary) {
				digicoind._paginateTxids.callCount.should.equal(1);
				digicoind._normalizeAddressArg.callCount.should.equal(1);
				digicoind.getAddressTxids.callCount.should.equal(1);
				digicoind.getAddressBalance.callCount.should.equal(1);
				digicoind._paginateTxids.args[0][1].should.equal(0);
				digicoind._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			digicoind.getAddressSummary(address, options, function(err, summary) {
				...
				digicoind.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					digicoind._paginateTxids.callCount.should.equal(0);
					digicoind._normalizeAddressArg.callCount.should.equal(2);
					digicoind.getAddressTxids.callCount.should.equal(1);
					digicoind.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			digicoind.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				digicoind._paginateTxids.callCount.should.equal(1);
				digicoind._normalizeAddressArg.callCount.should.equal(1);
				digicoind.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			digicoind._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		it('will give rpc error from client getblockhash', function(done) {
			...
			digicoind.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			sinon.spy(digicoind, '_tryAllClients');
			digicoind.nodes.push({
				client: {
					getBlock: sinon.stub().callsArgWith(2, { code: -1, message: 'Test error' })
				}
			});
			digicoind.getRawBlock(blockhash, function(err) {
				should.exist(err);
				digicoind._tryAllClients.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		});
		it('will try all nodes for getblock', function(done) {
			...
			digicoind.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			digicoind.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			digicoind.getBlock(0, function(err) {
				digicoind._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as digicoincore object from height', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			sinon.spy(digicoind, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			digicoind.getBlock(0, function(err, block) {
				should.not.exist(err);
				digicoind._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(digicoincore.Block);
				done();
			});
		});
		it('will getblock as digicoincore object', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			sinon.spy(digicoind, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			digicoind.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				digicoind._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache', function(done) {
			...
			digicoind.getBlock(hash, function(err, block) {
				should.not.exist(err);
				digicoind._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			digicoind.getBlock(0, function(err, block) {
				...
				digicoind.getBlock(0, function(err, block) {
					should.not.exist(err);
					digicoind._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			digicoind.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			var digicoind = new DigicoinService(baseConfig);
			sinon.spy(digicoind, '_maybeGetBlockHash');
			var getBlockHash = sinon.stub().callsArgWith(1, { code: -1, message: 'Test error' });
			digicoind.nodes.push({
				client: {
					getBlockHash: getBlockHash
			...
			digicoind.getBlockHeader(10, function(err) {
				digicoind._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			digicoind.getBlockHeader(blockhash, function(err) {
				digicoind._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			digicoind.getBlockHeader(0, function(err) {
				digicoind._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			digicoind.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				digicoind._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			digicoind.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				digicoind._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			digicoind._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				digicoind._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			digicoind._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			digicoind._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will handle error from maybeGetBlockHash', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			sinon.spy(digicoind, '_maybeGetBlockHash');
			digicoind._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
			digicoind.getBlockOverview(blockhash, function(err) {
				err.should.be.instanceOf(Error);
				digicoind._maybeGetBlockHash.callCount.should.equal(1);
				done();
			});
		...
		it('will give expected result', function(done) {
			...
			digicoind.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				digicoind.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					digicoind._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			digicoind.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		var tx = digicoincore.Transaction(txhex);
		it('will give rpc error', function() {
			...
			digicoind.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			digicoind.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			digicoind.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = digicoincore.Transaction();
			(function() {
				digicoind.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.getRawTransaction('txid', function(err) {
				should.exist(err);
				digicoind._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			digicoind.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.getTransaction('txid', function(err) {
				should.exist(err);
				digicoind._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			digicoind.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				digicoind._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(digicoincore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			digicoind.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				digicoind._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			digicoind.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getInfo and give result', function(done) {
			...
			digicoind.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			digicoind.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getInfo', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.getInfo(function(err) {
				should.exist(err);
				err.should.be.an.instanceof(errors.RPCError);
				done();
			});
		...
		it('will call client getInfo and give result', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.node.getNetworkName = sinon.stub().returns('testnet');
			...
			digicoind.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			digicoind.nodes.push({
				client: {
					getInfo: getInfo,
				},
			...
			digicoind.getInfo(function(err, info) {
				if (err) {
					return done(err);
				}

				digicoind.getSubsidy.callCount.should.equal(1);
				digicoind.client.getInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			digicoind.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		...
		it('will exit spawned process', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.spawn = {};
			digicoind.spawn.process = new EventEmitter();
			digicoind.spawn.process.kill = sinon.stub();
			digicoind.stop(done);
			digicoind.spawn.process.kill.callCount.should.equal(1);
			digicoind.spawn.process.kill.args[0][0].should.equal('SIGINT');
			digicoind.spawn.process.emit('exit', 0);
		});
		it('will give error with non-zero exit status code', function(done) {
			...
			digicoind.spawn.process.kill.callCount.should.equal(1);
			digicoind.spawn.process.kill.args[0][0].should.equal('SIGINT');
			digicoind.spawn.process.emit('exit', 1);
		});
		it('will stop after timeout', function(done) {
			var digicoind = new DigicoinService(baseConfig);
			digicoind.shutdownTimeout = 300;
			digicoind.spawn = {};
			digicoind.spawn.process = new EventEmitter();
			digicoind.spawn.process.kill = sinon.stub();
			digicoind.stop(function(err) {
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				digicoind._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			var digicoind = new DigicoinService(baseConfig);
			sinon.spy(digicoind, '_normalizeAddressArg');
			sinon.spy(digicoind, '_getBalanceFromMempool');
			var getAddressMempool = sinon.stub().callsArgWith(1, null, {
				result: deltas
			});
			digicoind.nodes.push({
			...
			digicoind.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				digicoind._getBalanceFromMempool.callCount.should.equal(1);
				digicoind._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				digicoind._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			digicoind.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				digicoind._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			digicoind.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			digicoind.getJsonBlock(0, function(err) {
				should.exist(err);
				digicoind._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			digicoind.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				digicoind._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			digicoind.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				digicoind._tryAllClients.callCount.should.equal(2);
				digicoind._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			digicoind.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				digicoind._tryAllClients.callCount.should.equal(1);
				digicoind._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will not add an invalid address', function() {
			var digicoind = new DigicoinService(baseConfig);
			var emitter = new EventEmitter();
			digicoind.subscribeBalance(emitter, ['invalidaddress']);
			should.not.exist(digicoind.subscriptions.balance['invalidaddress']);
			log.info.callCount.should.equal(1);
		});
		it('will add a valid address', function() {
			var digicoind = new DigicoinService(baseConfig);
			var emitter = new EventEmitter();
			digicoind.subscribeBalance(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(1);
		...
		it('will handle multiple address subscribers', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			digicoind.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			digicoind.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			digicoind.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			digicoind.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			digicoind.unsubscribeBalance(emitter1);
			digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			digicoind.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			digicoind.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var digicoind = new DigicoinService(baseConfig);
			...
			digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			digicoind.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			digicoind.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var digicoind = new DigicoinService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			digicoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(digicoind, 'unsubscribeBalanceAll');
			digicoind.unsubscribeBalance(emitter1);
			digicoind.unsubscribeBalanceAll.callCount.should.equal(1);
			digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			digicoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			digicoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			digicoind.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			digicoind.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			digicoind.unsubscribeBalanceAll(emitter1);
			digicoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			digicoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			digicoind.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(digicoind.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			var digicoind = new DigicoinService(baseConfig);
			sinon.spy(digicoind, '_notifyBalanceSubscriber');
			digicoind._getAddressesFromTransaction = sinon.stub().returns(addresses);
			digicoind.getAddressSummary = sinon.stub().callsArgWith(2, null, {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				digicoind._notifyBalanceSubscriber.callCount.should.equal(1);
				digicoind._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			digicoind._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				digicoind.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			digicoind._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			digicoind.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				digicoind._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.getDgpInfo(function(err, result) {
				should.exist(err);
				digicoind._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.getMiningInfo(function(err, result) {
				should.exist(err);
				digicoind._tryAllClients.callCount.should.equal(1);
	...
	describe('#getStakingInfo', function() {
		it('will give rpc error', function(done) {
			...
			digicoind.getStakingInfo(function(err, result) {
				should.exist(err);
				digicoind._tryAllClients.callCount.should.equal(1);
